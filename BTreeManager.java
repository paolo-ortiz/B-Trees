...
public class BTreeManager {
	...
	static long[] BTreeValues = new long[14]; //change from initialBTreeValues to BTreeValues
...
//-------------------------------------------------------------------------------------
	...
	BTreeManager(String name) throws IOException {
		...
		//else if first time creating data.bt
			...
			//moved this here to free up the writeValues
...
//-------------------------------------------------------------------------------------
	...
	public static void splitRootNode(BTreeNode node, long lastKey, long lastValueIndex) throws IOException {
		...
		for (int i = 0; i < 14; i++) {
			//hardcoding for now to look good
			if(i == 2){
				rightChildValues[i] = tempBTreeValues[i+9];
				rightChildValues[i+1] = tempBTreeValues[i+10];
			}
			
			else if(i == 5){
				rightChildValues[i] = lastKey;
				rightChildValues[i+1] = lastValueIndex;
			}
		...
		//putting the middle into the root
		long[] root = new long[14];
		
		for (int i = 0; i < 14; i++) {
			if (i == 2) { //if at 1st key
				root[i] = tempBTreeValues[i + 6]; //set key
				leftChildValues[i + 1] = tempBTreeValues[i + 7]; //set value index
			}
...
//-------------------------------------------------------------------------------------
	...
	public static void writeNewNode(long[] BTreeValues) throws IOException {
		...
		//write values
		writeValuesToBTree(seekLocation, BTreeValues);
